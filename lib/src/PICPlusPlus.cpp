...
PICPlusPlus::PICPlusPlus(DATA_STRUCTS::InputVariables inputVariables)
    ...
    : m_simulationParams(inputVariables.simulationParams),
      ...
      m_rhos(inputVariables.simulationParams.numSpecies,
             std::vector<double>(m_simulationParams.numGrid + 1, 0.0)),
      m_rho0(m_simulationParams.numSpecies,
             std::vector<double>(m_simulationParams.numGrid + 1, 0.0)),

      m_chargeDensity(m_simulationParams.numGrid + 1, 0.0),
      m_electricField(m_simulationParams.numTimeSteps + 1,
                      std::vector<double>(m_simulationParams.numGrid + 1, 0.0)),
      m_particleAcceleration(m_simulationParams.numGrid + 1),

      m_particleKineticEnergy(
          m_simulationParams.numSpecies,
          std::vector<double>(m_simulationParams.numTimeSteps + 1, 0.0)),
      m_electrostaticEnergy(m_simulationParams.numTimeSteps + 1, 0.0),
      m_totalEnergy(m_simulationParams.numTimeSteps + 1, 0.0) {

  m_simulationParams.gridStepSize =
      m_simulationParams.spatialLength / m_simulationParams.numGrid;
  ...
  for (int species = 0; species < m_simulationParams.numSpecies; species++) {
    ...
    speciesData.particleCharge =
        m_simulationParams.spatialLength * speciesData.plasmaFrequency *
        speciesData.plasmaFrequency /
        (speciesData.numParticles * speciesData.chargeMassRatio);
    speciesData.particleMass =
        speciesData.particleCharge / speciesData.chargeMassRatio;
    speciesData.chargeCloudWidth =
        m_simulationParams.spatialLength / speciesData.numParticles;

    initializePositions(speciesData.particlePositions, speciesData.numParticles,
                        speciesData.chargeCloudWidth);
    initializeVelocities(speciesData.particleXVelocities,
                         speciesData.numParticles, speciesData.driftVelocity,
                         speciesData.thermalVelocity);
    ...
    if (m_allSpeciesData[species].spatialPerturbationAmplitude != 0) {
      applySpatialPerturbation(
          m_allSpeciesData[species].particlePositions,
    ...
    m_qdx[species] = m_allSpeciesData[species].particleCharge /
                     m_simulationParams.gridStepSize;

    setRho(species, m_simulationParams, m_allSpeciesData, m_qdx,
           m_chargeDensity, m_rho0, m_rhos);
  }
...
std::optional<nlohmann::json> PICPlusPlus::initialize() {
  ...
  fields(m_simulationParams, m_chargeDensity, m_electricField, m_timeStep,
         m_particleAcceleration, m_ael);

  accel(m_simulationParams, m_allSpeciesData, m_particleAcceleration,
        m_timeStep, m_ael);
  ...
  for (m_timeStep = 1; m_timeStep <= m_simulationParams.numTimeSteps;
       m_timeStep++) {
    runTimeLoop();
  }
...
void PICPlusPlus::runTimeLoop() {
  ...
  Flat2DVector<size_t> f(2, 2);
  size_t i = 1;
  f[0][0] = i;
  std::cout << "THE CURRENT SHIT IS " << f[0][0];
  accel(m_simulationParams, m_allSpeciesData, m_particleAcceleration,
        m_timeStep, m_ael);

  move(m_simulationParams, m_allSpeciesData, m_chargeDensity, m_rho0, m_qdx);
...
void PICPlusPlus::initializePositions(
    std::vector<double> &inOutParticlePositions, const int numParticles,
...
void PICPlusPlus::addDriftVelocity(
    std::vector<double> &inOutParticleXVelocities, const int numParticles,
...
void PICPlusPlus::addThermalVelocity(
    std::vector<double> &inOutParticleXVelocities, const int numParticles,
...
void PICPlusPlus::initializeVelocities(
    std::vector<double> &inOutParticleXVelocities, const int numParticles,
    const double driftVelocity, const double thermalVelocity) {
  if (driftVelocity != 0) {
...
void PICPlusPlus::applySpatialPerturbation(
    std::vector<double> &inOutParticlePositions, const int numParticles,
    const int spatialPerturbationMode,
    const double spatialPerturbationAmplitude) {

  double spatialPertConst = 2 * std::numbers::pi * spatialPerturbationMode /
                            m_simulationParams.spatialLength;

  for (int a = 0; a < numParticles; ++a) {
    double theta = spatialPertConst * inOutParticlePositions[a];
    inOutParticlePositions[a] =
        inOutParticlePositions[a] + spatialPerturbationAmplitude * cos(theta);

    if (inOutParticlePositions[a] >= m_simulationParams.spatialLength) {
      inOutParticlePositions[a] =
          inOutParticlePositions[a] - m_simulationParams.spatialLength;
    }
...
void PICPlusPlus::calculateEnergies() {
  ...
  for (int species = 0; species < m_simulationParams.numSpecies; species++) {
    for (int i = 0; i < m_allSpeciesData[species].numParticles; i++) {
      m_particleKineticEnergy[species][m_timeStep] +=
          0.5 *
          std::pow((m_allSpeciesData[species].particleXVelocities[i]), 2) *
          m_allSpeciesData[species].particleMass;
    }
  }
  ...
  for (int i = 0; i < m_simulationParams.numGrid + 1; i++) {
    m_electrostaticEnergy[m_timeStep] +=
        std::pow(m_electricField[m_timeStep][i], 2) * halfGridSize;
  }
}
